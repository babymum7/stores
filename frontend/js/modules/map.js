...
function createErrorElement() {
  ...
  return text => {
    ...
    requestAnimationFrame(() => {
      ...
      errorElement.style = 'transition: opacity 3s linear';
...
function loadPlaces(map, lat = 21.0031177, lng = 105.82014079999999, infoWindow, topStore) {
  infoWindow.close();
  ...
  axios
    ...
    .then(res => {
      ...
      autocomplete.addListener('place_changed', () => {
        ...
        if (place.geometry) {
          loadPlaces(map, place.geometry.location.lat(), place.geometry.location.lng(), infoWindow);
        }
      });
      $('#findPlacesNearYou').on('click', () => {
        if ('geolocation' in navigator) {
          ...
          errorMessage('Your brower dose not support that feature');
        }
      });
